...
class Gateway(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if system.startswith('Extract'):
            ...
            claim = {'text': text, 'quote': source if not source.startswith('POISON') else 'not in source',
...
class SystemTests(unittest.TestCase):
    ...
    def test_06_redaction_all_projections_receipts_and_rebuilds(self):
        ...
        receipt = self.find('Sensitive removable canary')['receipt_id']
        ...
        self.assertEqual(self.call('/v1/receipts/' + receipt)['claim_ids'], [])
